<template>
  <div>
    ...
    <el-upload
      ...
      :before-upload="quillFileBefore"
      accept='.zip,.rar,.xls,.xlsx,.doc,.docx,.ppt,.pptx'
    ></el-upload>
...
const toolbarOptions = [
  ...
  [{ align: [] }],                                 // 对齐方式
  ["clean"],                                       // 清除文本格式
  ["link", "image", "video","file"]                       // 链接、图片、视频
];
          ...
          toolbar: {
            ...
            handlers: {
              ...
              file:function(value){
                if(value){
                  //触发选择附件
                  document.querySelector(".quill-file input").click();
                }else{
  ...
  methods: {
    ...
    quillImgSuccess(res, file) {
      ...
      if (res.code == 0) {
        ...
        quill.insertEmbed(length, "image",  process.env.VUE_APP_BASE_API+ '/' +res.data.fileInfo.fileUrl);
        // 调整光标到最后

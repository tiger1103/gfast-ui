<template>
	<div class="system-menu-container layout-pd">
					...
					<el-table-column label="操作" show-overflow-tooltip width="160">
						<template #default="scope">
							<el-button v-if="scope.row.menuType !== 2" size="small" link type="primary"
								@click="onOpenAddMenu('add', scope.row)" v-auth="'api/v1/system/menu/add'">新增</el-button>
							<el-button size="small" link type="primary" @click="onOpenEditMenu('edit', scope.row)"
		...
		<MenuDialog ref="menuDialogRef" @refresh="menuList" :visibleOptions="sys_show_hide" />
	</div>
</template>
...
// 打开新增菜单弹窗
const onOpenAddMenu = (type: string,row: SysMenuItem | undefined = undefined) => {
	menuDialogRef.value.openDialog(type,row);
};
// 打开编辑菜单弹窗
const onOpenEditMenu = (type: string,row: SysMenuItem) => {
	menuDialogRef.value.openDialog(type,row);
};
// 删除当前行
	...
	ElMessageBox.confirm(`此操作将永久删除路由：${row.path}, 是否继续?`, '提示', {
		...
		.then(() => {
			delMenu(row.id).then(() => {
				...
				menuDialogRef.value.resetMenuSession();
				menuList();
			});
		})
		.catch(() => { });

<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="configList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="系统内置" align="center" prop="config_type" :formatter="typeFormat" />
...
export default {
  ...
  methods: {
    ...
    typeFormat(row, column) {
      return this.selectDictLabel(this.typeOptions.values, row.config_type);
    },
    // 取消按钮
    cancel() {
      this.open = false;
    ...
    handleUpdate(row) {
      ...
      getConfig(configId).then(response => {
        this.form = {
          ...
          configName: response.data.config_name,
          configKey: response.data.config_key,
          configValue: response.data.config_value,
          configType: ""+response.data.config_type,
